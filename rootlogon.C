...
void ShowPrint(TString channel_name, TCut user_cut="", TString outfileName = ""){
  TTree* run_tree = (TTree*)gDirectory->Get("T");  
  TString dv_list[]=
    {"asym_usl",
     "asym_usr",
  ...
  if(channel_name.Contains("mean")){
	...
	fGraph->GetXaxis()->SetBinLabel(ipt+1,
      ...
      // Textfile append on the same line (not a new line) (.... , (be careful that you always use the same nomenclature %channel_name,)%stuff);
    }
    c1->Print(Form("showPrint_%s.pdf",outfileName.Data()));
}
